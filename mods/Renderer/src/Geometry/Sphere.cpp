...
Sphere::Sphere(const glm::vec3 &center, float radius, const std::shared_ptr<FMaterial> &mat)
    : center_(center), radius(radius), radius2(radius * radius), material_(mat), area(4 * M_PI_DEFINED * radius * radius)
{
}
...
void Sphere::transform(const glm::mat4 &trans) {
    ...
    area = 4 * M_PI_DEFINED * radius * radius;
...
bool Sphere::hasEmit() const {
    if (auto&& mat = material_.lock()) { return mat->hasEmission(); }
    return false;
}
