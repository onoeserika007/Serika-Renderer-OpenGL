...
std::shared_ptr<UMesh> ResourceLoader::processMesh(aiMesh *mesh, const aiScene *scene) {
	...
	// if null, do not set
	if (!posAttrs.empty()) pgeomerty->setAttribute(EBA_Position, posAttrs, true);
	if (!texcoordAttrs.empty()) pgeomerty->setAttribute(EBA_TexCoord, texcoordAttrs);
	if (!normalAttrs.empty()) pgeomerty->setAttribute(EBA_Normal, normalAttrs);
	// 处理索引
	...
	if (static_cast<int>(mesh->mMaterialIndex) >= 0)
	{
		...
		for (int i = 0; i <= aiTextureType_TRANSMISSION; i++) {
			...
			for (unsigned int j = 0; j < material->GetTextureCount(type); j++)
			{
				...
				pmaterial->setTextureData(static_cast<TextureType>(i), picture);
			}
		}
	}
...
std::shared_ptr<UMesh> ResourceLoader::loadSkyBox(const std::string &path) {
	...
	TextureData texData;
	texData.unitDataArray = std::move(skyboxTex);
